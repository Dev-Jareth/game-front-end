...
export class Input extends Component {
  ...
  handleKeyPress = e => (this.props.onKeyPress ? this.props.onKeyPress(e.key) : void 0);
  determineInputType = () => (this.props.type === "password" ? "password" : "text");

  render = () => {
    let icon = this.props.icon ? (
      <label className={`input-icon fa fa-${this.props.icon}`} htmlFor="login-name" />
    ) : null;
