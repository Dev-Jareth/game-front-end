...
export default (ship = "ship", camera, canvas) => {
  ...
  loader.load(`./models/${ship}.json`, (geo, mat) =>
    player.add(new THREE.Mesh(geo, mat || new THREE.MeshStandardMaterial({ color: 0xffffff })))
  );
  ...
  loader.load(`./models/${ship}_engine2.json`, (geo, mat) =>
    engines.add(new THREE.Mesh(geo, mat || new THREE.MeshStandardMaterial({ color: 0xffffff })))
  );
  //CameraControls
  this.enabled = false;
  let maxCameraY = 0.25;
  let maxCameraX = 0.5;
  let convergeOnZero = value => (Math.abs(value) > 0.001 ? value * 0.9995 : 0);
  let resetting = false;
  let resetCamera = () => {
    resetting = true;
    if (cameraPan.rotation.y === 0 && cameraWrapper.rotation.x === 0) return (resetting = false);
    cameraPan.rotation.y = convergeOnZero(cameraPan.rotation.y);
    cameraWrapper.rotation.x = convergeOnZero(cameraWrapper.rotation.x);
  ...
  document.addEventListener("mousemove", e => {
    if (this.enabled) {
      ...
      cameraWrapper.rotation.x = Math.min(
        maxCameraY,
        Math.max(-maxCameraY, cameraWrapper.rotation.x - e.movementY / 1000)
      );

      if (!resetting) resetCamera();
    }
  });
